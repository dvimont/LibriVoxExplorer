 ...
 * The specific class indexed by the CompositeIndex 
 ...
 * {@link Key} interface. 
 An CompositeIndex works most effectively when "autofill"
 ...
 * <i>(Note that, depending upon IDE and compiler settings, it may be
 ...
 when instantiating instances of CompositeIndex, in order to avoid 
 "unchecked generic array creation for varargs parameter" warnings 
 during compilation.)</i>
 */
public class CompositeIndex<V> {
    ...
    private final Map<Key, CompositeIndex<V>> middleMap = new TreeMap<>();
     ...
     */
    @SafeVarargs
    public CompositeIndex(Class<? extends V> valueClass,
                        String title, 
     ...
     */
    @SafeVarargs
    public CompositeIndex(String title, 
     ...
     */
    @SafeVarargs
    protected CompositeIndex(int dummyInt,
                        Class<?> valueClass,
                        String title, 
                        Class<? extends Key>... keyClassArray) {
        this(true, valueClass, title, keyClassArray);
     ...
     */
    @SafeVarargs
    private CompositeIndex(boolean isTopLevelMap,
                        Class<?> valueClass,
                        String title, 
                        Class<? extends Key>... keyClassArray) {
    ...
    /**
     * Add an entry to this CompositeIndex. (Analogous to put method of a 
     ...
     * @throws InvalidKeyException if any {@link Key} object in the
     * {@link MultiKeyEntry}'s {@link KeyArray} is not of the correct type 
 and position as denoted by this CompositeIndex's 
 {@link IndexedCollection#getKeyClassArray() KEY_CLASS_ARRAY}
     */
    @SafeVarargs
 ...
 {@link IndexedCollection#getKeyClassArray() KEY_CLASS_ARRAY}
    ...
    public final boolean put (MultiKeyEntry<V> multiKeyEntry) 
        ...
        if (multiKeyEntry.getKeyArrayLength() == 1) {
            ...
            CompositeIndex<V> lowerIndexNode 
            ...
            if (lowerIndexNode == null) {
                this.middleMap.put
                    (multiKeyEntry.getTopKey(), 
                        new CompositeIndex<V>(this.VALUE_CLASS, this.TITLE, 
    ...
    /**
     * Get the "depth" of this CompositeIndex as denoted by the number of 
 entries in its {@link IndexedCollection#KEY_CLASS_ARRAY KEY_CLASS_ARRAY}.
     * @return The "depth" of this CompositeIndex as denoted by the number of 
 entries in its {@link IndexedCollection#KEY_CLASS_ARRAY KEY_CLASS_ARRAY}.
     */
    public int getDepth () {
    ...
    /**
     * Get this CompositeIndex's {@link IndexedCollection#TITLE TITLE}
    ...
    /**
     * Get this CompositeIndex's {@link IndexedCollection#KEY_CLASS_ARRAY KEY_CLASS_ARRAY}
    ...
    /**
     * Invoked to get all values contained in this CompositeIndex, in the 
 order in which they are indexed.
     * @return All values contained in this CompositeIndex, in the order in
 which they are indexed.
    ...
    public List<V> selectAll () {
        ...
        if (middleMap != null && !middleMap.isEmpty()) {
            for (CompositeIndex<V> lowerIndexNode : this.middleMap.values()) {
    ...
    public List<V> get (KeyArray keyArray){
        ...
        if (bottomMap != null && !bottomMap.isEmpty()) {
            if (keyArray.getTopKey() == null) {
                ...
                // checking for null -- fix added 2015-04-06
                V bottomMapValue = this.bottomMap.get(keyArray.getTopKey());
                if (bottomMapValue != null) {
                    vList.add(bottomMapValue);
                }
            }
        } 
        if (middleMap != null && !middleMap.isEmpty()) {
    ...
    /**
     * Get Class of the values indexed in this CompositeIndex
     * @return Class of the values indexed in this CompositeIndex
     */
    ...
    /**
     * Size of this CompositeIndex, as denoted by the number of unique value
 objects it contains
     * @return Size of this CompositeIndex, as denoted by the number of 
 unique value objects it contains
    ...
    public int size() {
        ...
        if (middleMap != null && !middleMap.isEmpty()) {
            for (CompositeIndex<V> lowerIndex : this.middleMap.values()) {
    ...
    private Set<MultiKeyEntry<V>> getMultiKeyEntrySet() {
        ...
        if (middleMap != null && !middleMap.isEmpty()) {
            for (Entry<Key, CompositeIndex<V>> entry 
    ...
    /**
     * Indicates whether this CompositeIndex is enabled for auto-fill.
    ...
    /**
     * Prints verbose listing of CompositeIndex contents; 
