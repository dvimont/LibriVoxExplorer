 ...
 * @param <V>
 */
public class IndexedCollection<V> extends AbstractIndexedCollection<V> {
    
    private IndexedKeyCollection<IndexedKey> indexedKeyDirectory;
    private CompositeIndex<CompositeIndex<IndexedKey>> 
                                                metaCollectionForIndexedKeys;
     ...
     * @param masterClass
     */
    public IndexedCollection (Class<V> masterClass) {
        super(masterClass);
    }
    ...
    public final void buildIndexedKeyDirectory 
        (String title, CompositeIndex<IndexedKey>... indexedCollectionArray) 
                throws InvalidKeyException,
                        CompositeIndexBuildFailureException {
        CompositeIndex.checkVarargs(indexedCollectionArray);
        indexedKeyDirectory 
                = new IndexedKeyCollection<IndexedKey>(IndexedKey.class);
    ...
    private final void buildDirectoryOfGetMethodsForIndexedKeys()
        ...
        metaCollectionForIndexedKeys 
            = new CompositeIndex<CompositeIndex<IndexedKey>>
                    (0, CompositeIndex.class, 
                        "MetaCollection to match MASTER_CLASS getMethods "
                                + "with IndexedCollections of IndexedKeys",
                        MethodKey.class, IntegerKey.class);
        int uniqueId = 0;
        for (CompositeIndex<IndexedKey> indexedCollection 
    ...
    public final void autoFill (V value, Key valueKey) 
        ...
        for (CompositeIndex<V> indexedCollection : this.metamap.selectAll()) {
            if (indexedCollection.autofillEnabled() 
    ...
    private void autoFillIndexedKeyMaps (V value) 
        ...
        for (Method thisGetMethod : ACTIVE_GET_METHODS) {
            List<CompositeIndex<IndexedKey>> mapsRelatedToThisGetMethod 
                    = metaCollectionForIndexedKeys
     ...
     * @return
     */
    public AbstractIndexedCollection<IndexedKey> getIndexedKeyDirectory () {
